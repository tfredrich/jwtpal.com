...
function setPayloadToggleAvailability(canUseSdJwt) {
  ...
  if (jwtLabel) jwtLabel.classList.remove("disabled");
  if (sdInput) sdInput.disabled = !canUseSdJwt;
  if (sdLabel) sdLabel.classList.toggle("disabled", !canUseSdJwt);
}
...
function updateSdJwt(decoded, rawJwt) {
  ...
  if (!isSdJwtHeader(decoded.header)) {
    resetSdJwtUI();
    setPayloadToggleAvailability(false);
    setPayloadMode("jwt");
    return Promise.resolve();
  }
  resetSdJwtUI();
  setPayloadToggleAvailability(true);
...
window.onload = function() {
  ...
  if (sdToggle) {
    sdToggle.addEventListener("change", function() {
      if (!sdToggle.disabled && sdToggle.checked) setPayloadMode("sd-jwt");
    });
  }
  if (jwtLabel) {
  ...
  if (sdLabel) {
    sdLabel.addEventListener("click", function() {
      if (sdToggle && sdToggle.disabled) return;
      setPayloadMode("sd-jwt");
    });
  }
